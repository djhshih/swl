...
def _compute_lambda_captures(checker, expr, outer_scope, local_scope):
    from swl.semantic.wf.scope import collect_name_refs
    refs = collect_name_refs(checker, expr.body)
    local_names = {expr.param.name}
    if expr.body.type == wf_node.NodeType.block:
        for item in expr.body.body:
            if item.type == wf_node.NodeType.bind:
                local_names.add(item.id.name)
    captures = {}
    for ref in refs:
        if ref not in local_names and outer_scope.resolve(ref) is not None:
            binding = outer_scope.resolve(ref)
            ir_node = getattr(binding, 'ir_node', None)
            if ir_node is not None:
                captures[ref] = ir_node
    return captures


def _lower_block_items(lowerer, items, scope, imports):
    bindings = []
    local_scope = Scope(parent=scope)
    for item in items[:-1]:
        if item.type != wf_node.NodeType.bind:
            continue
        value = lowerer.lower_binding(item, local_scope, imports)
        var = ir.Variable(lowerer._alloc_var_id(), item.id.name, value)
        local_scope.set_local(item.id.name, ir_node=ir.Ref(var.id, var.name))
        bindings.append(var)
    return bindings, local_scope, lowerer.lower_expr(items[-1], local_scope, imports)
...
class Lowerer:
    ...
    def lower_file(self, path: str):
        ...
        self._lowering_dir = os.path.dirname(path)
    ...
    def lower_tree(self, tree, imports, signature=None):
        ...
        old_dir = self._lowering_dir
        self.signature = signature
        try:
            return self._lower_tree_impl(tree, imports, signature)
        finally:
            self.signature = old_sig
            self._lowering_dir = old_dir

    def _lower_tree_impl(self, tree, imports, signature=None):
        scope = Scope()
        if tree.type != wf_node.NodeType.block:
            return self.normalize(self._ensure_block_root(self.lower_expr(tree, scope, imports), signature))

        bindings, scope, result = _lower_block_items(self, tree.body, scope, imports)
        if isinstance(result, ir.Lambda) and signature is not None:
            result = ir.Lambda(result.param, result.body, signature, getattr(result, 'is_batch', False), captures=result.captures)
        if not bindings:
            return _normalize_twice(self, result)
        return _normalize_twice(self, ir.Block(bindings, result))

    def lower_binding(self, expr, scope, imports):
        if expr.type == wf_node.NodeType.bind and expr.id.name in imports and scope.resolve(expr.id.name) is None:
            imported = imports[expr.id.name]
            return self._function_from_import(expr.id.name, imported)
        return self.lower_expr(expr.value, scope, imports)

    def _lower_name(self, name, scope, imports):
        binding = scope.resolve(name)
        if binding is not None:
            return getattr(binding, 'ir_node', None)
        if name in imports:
    ...
    def lower_expr(self, expr, scope, imports):
        if expr.type == wf_node.NodeType.id:
            return self._lower_name(expr.name, scope, imports)
        ...
        if expr.type == wf_node.NodeType.rec:
            ...
            for name, value in expr.value.items():
                fields[name] = self.lower_expr(value, scope, imports)
            return ir.Record(fields)

        if expr.type == wf_node.NodeType.get:
            return ir.Field(self.lower_expr(expr.rec, scope, imports), expr.member.name)

        if expr.type == wf_node.NodeType.update:
            return ir.Update(
                self.lower_expr(expr.left, scope, imports),
                self.lower_expr(expr.right, scope, imports),
            )
        ...
        if expr.type == wf_node.NodeType.apply:
            ...
            if builtin == 'map_apply':
                ...
                return ir.Map(
                    self.lower_expr(function_expr, scope, imports),
                    self.lower_expr(arg_expr, scope, imports),
                )
            ...
            if builtin == 'map_partial':
                return ir.Apply(
                    ...
                    self.lower_expr(expr.arg, scope, imports),
                )
            ...
            return ir.Apply(
                self.lower_expr(expr.fun, scope, imports),
                self.lower_expr(expr.arg, scope, imports),
                satisfied=satisfied,
            )
        ...
        if expr.type == wf_node.NodeType.fun:
            local_scope = Scope(parent=scope)
            local_scope.declare(expr.param.name, ir_node=ir.Name(expr.param.name))
            body = self.lower_expr(expr.body, local_scope, imports)
            captures = _compute_lambda_captures(self.checker, expr, scope, local_scope)
            return ir.Lambda(expr.param.name, self._ensure_block(body), captures=captures)

        if expr.type == wf_node.NodeType.block:
            bindings, _, result = _lower_block_items(self, expr.body, scope, imports)
            if not bindings:
                return result
            return ir.Block(bindings, result)

        if expr.type == wf_node.NodeType.chain:
            items = self._lower_chain_items(expr, scope, imports)
            return self._chain_to_lambda_block(items, None)

        if expr.type == wf_node.NodeType.bind:
            value = self.lower_binding(expr, scope, imports)
    ...
    def _lower_inline_import(self, path):
        ...
        base_dir = self._lowering_dir if self._lowering_dir else '.'
    ...
    def _function_from_import(self, name, imported):
        if imported.path in self.function_cache:
            return self.function_cache[imported.path]
        ...
        self.function_cache[imported.path] = function
        return function
    ...
    def _lower_chain_items(self, expr, scope, imports):
        if expr.type != wf_node.NodeType.chain:
            return [self.lower_expr(expr, scope, imports)]
        return self._lower_chain_items(expr.left, scope, imports) + self._lower_chain_items(expr.right, scope, imports)

    def normalize(self, node):
        if isinstance(node, ir.Lambda):
            body = self.normalize(node.body)
            staged = self._normalize_block_lambda(node.param, body, node.signature, node.captures)
            if staged is not None:
                return staged
            return ir.Lambda(node.param, body, node.signature, captures=node.captures)
    ...
    def _normalize_block_lambda(self, param, body, signature, captures=None):
        if not isinstance(body, ir.Block):
            return None
        return ir.Lambda(param, body, signature, captures=captures or {})
    ...
    def _ensure_block_root(self, node, signature):
        if isinstance(node, ir.Lambda):
            ...
            return ir.Lambda(node.param, self._ensure_block(node.body), sig, captures=node.captures)
        return node
    ...
    def _normalize_with_bindings(self, node, binding_map):
        ...
        if isinstance(node, ir.Apply):
            ...
            if isinstance(fn, ir.Ref) and fn.id in binding_map:
                fn = binding_map[fn.id]
            return ir.Apply(fn, self._normalize_with_bindings(node.arg, binding_map), node.signature)
        if isinstance(node, ir.Record):
        ...
        if isinstance(node, ir.Lambda):
            body = self._normalize_with_bindings(node.body, binding_map)
            return ir.Lambda(node.param, body, node.signature, captures=node.captures)
        if isinstance(node, ir.Block):
            inner = {bind.id: bind.value for bind in node.bindings}
            merged = dict(binding_map)
            merged.update(inner)
            raw_bindings = [ir.Variable(b.id, b.name, self.normalize(b.value)) for b in node.bindings]
            bindings = [ir.Variable(b.id, b.name, self._normalize_with_bindings(b.value, merged)) for b in raw_bindings]
            merged2 = dict(binding_map)
            merged2.update({b.id: b.value for b in bindings})
            return ir.Block(bindings, self._normalize_with_bindings(node.result, merged2))
        return node
    ...
    def _generated_callable_from_lambda(self, node):
        if not isinstance(node, ir.Lambda):
            return None
        helper = ir.Lambda(node.param, self.normalize(node.body), node.signature, captures=node.captures)
        ...
        return ir.Function(
            ...
            signature,
            f'<generated:{name}>',
            body=ir.Lambda(node.param, helper.body, signature, captures=node.captures),
        )
...
def parse_and_lower(src: str, base_dir: str = '.', files=None):
    ...
    if result.errors:
        raise ValueError('\n'.join(result.errors))
    lowerer = Lowerer(files=files)
    lowerer._lowering_dir = base_dir
    return lowerer.lower_tree(result.tree, result.imports, result.signature)
