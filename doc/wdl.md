...
1. **Custom data feed**: The mapped sub-workflow expects `Array[Struct]` input, where the struct matches the workflow's input schema. Users must provide a custom data feed to convert their data (e.g., a record of arrays) into the required `Array[Struct]` format. This conversion is not auto-generated by the transpiler.
